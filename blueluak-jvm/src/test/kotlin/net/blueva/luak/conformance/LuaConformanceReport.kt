 ...
 *
 * ```
 * ./gradlew :blueluak-jvm:test -Dblueluak.lua.testsuite=/path/to/lua/testes
 * ```
 *
...
class LuaConformanceReport {
    ...
    private fun render(suite: File, results: List<Result>): String = buildString {
        ...
        appendLine("BlueLuaK conformance against the Lua reference suite")
        appendLine("suite:     $suite")
        appendLine("_VERSION:  ${Lua._VERSION}   (${Lua.BLUELUAK_VERSION})")
    ...
    private companion object {
        const val SUITE_PROPERTY = "blueluak.lua.testsuite"
