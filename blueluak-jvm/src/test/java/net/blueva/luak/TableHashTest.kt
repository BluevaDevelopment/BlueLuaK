...
class TableHashTest : TestCase() {
    ...
    fun testSetRemove() {
        ...
        val keys = arrayOf<String?>(
            "abc", "def", "ghi", "jkl", "mno", "pqr", "stu", "wxy", "z01",
            "cd", "ef", "g", "hi", "jk", "lm", "no", "pq", "rs",
        )
        val capacities = intArrayOf(0, 2, 2, 4, 4, 8, 8, 8, 8, 16, 16, 16, 16, 16, 16, 16, 16, 32, 32, 32)
        for (i in keys.indices) {
            TestCase.assertEquals(capacities[i], t.getHashLength())
            val si = "Test Value! " + i
            t.set(keys[i], si)
            TestCase.assertEquals(0, t.length())
            TestCase.assertEquals(i + 1, t.keyCount())
        }
        TestCase.assertEquals(capacities[keys.size], t.getHashLength())
        for (i in keys.indices) {
            val vi: LuaValue = LuaString.valueOf("Test Value! " + i)
        ...
        for (i in keys.indices) {
            val vi: LuaValue = LuaString.valueOf("Replacement Value! " + i)
            assertEquals(vi, t.get(keys[i]))
        }
        ...
        for (i in keys.indices) {
            ...
            if (i < keys.size - 1) TestCase.assertEquals(capacities[keys.size], t.getHashLength())
    ...
    fun testIndexFunction() {
        ...
        val fb: TwoArgFunction = object : TwoArgFunction() {
    ...
    fun testLoopWithRemoval() {
        ...
        while (!entry.isnil(1)) {
            val k: LuaValue = entry.arg1()!!
            val v: LuaValue = entry.arg(2)!!
            if ((k.toint() and 1) == 0) {
