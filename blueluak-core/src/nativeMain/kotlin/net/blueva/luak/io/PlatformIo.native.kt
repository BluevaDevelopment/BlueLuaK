...
private class NativeFileOutputStream(
...
@OptIn(ExperimentalForeignApi::class)
private val nativeStandardOutput = PrintStream(NativeFileOutputStream(stdout!!))

@OptIn(ExperimentalForeignApi::class)
private val nativeStandardError = PrintStream(NativeFileOutputStream(stderr!!))

actual fun standardOutput(): PrintStream = nativeStandardOutput
actual fun standardError(): PrintStream = nativeStandardError

@OptIn(ExperimentalForeignApi::class)
actual fun platformResource(name: String): InputStream? {
    val file = fopen(name, "rb") ?: return null
    return try {
...
private const val RESOURCE_BUFFER_SIZE = 8 * 1024
