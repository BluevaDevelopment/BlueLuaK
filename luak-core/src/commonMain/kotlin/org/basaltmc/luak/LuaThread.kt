 ...
 *
 *
 * The utility classes [org.basaltmc.luak.lib.LuaPlatform] and
 ...
 *
 * @see org.basaltmc.luak.lib.CoroutineLib
 */
/**
...
class LuaThread : LuaValue {
    ...
    constructor(globals: Globals) {
        state = org.basaltmc.luak.LuaThread.State(globals, this, null)
        state.status = org.basaltmc.luak.LuaThread.Companion.STATUS_RUNNING
    ...
    constructor(globals: Globals, func: LuaValue?) {
        ...
        state = org.basaltmc.luak.LuaThread.State(globals, this, func)
        this.globals = globals
    }
    ...
    override fun getmetatable(): LuaValue? {
        return org.basaltmc.luak.LuaThread.Companion.s_metatable
    }

    val status: String?
        get() = org.basaltmc.luak.LuaThread.Companion.STATUS_NAMES[state.status]
    ...
    fun resume(args: Varargs?): Varargs {
        val s = this.state
        if (s.status > org.basaltmc.luak.LuaThread.Companion.STATUS_SUSPENDED) return LuaValue.varargsOf(
            LuaValue.FALSE,
            LuaValue.valueOf("cannot resume " + (if (s.status == org.basaltmc.luak.LuaThread.Companion.STATUS_DEAD) "dead" else "non-suspended") + " coroutine")
        )
    ...
    fun close(): Varargs {
        ...
        val s = this.state
        if (s.status == org.basaltmc.luak.LuaThread.Companion.STATUS_NORMAL) {
            LuaValue.error("cannot close a normal coroutine")
        }
        if (s.status == org.basaltmc.luak.LuaThread.Companion.STATUS_RUNNING) {
    ...
    class State internal constructor(globals: Globals, lua_thread: LuaThread, function: LuaValue?) {
        ...
        var status: Int = org.basaltmc.luak.LuaThread.Companion.STATUS_INITIAL
        ...
        private val completion = object : Continuation<Varargs> {
            ...
            override fun resumeWith(result: Result<Varargs>) {
                finished = true
                finalResult = result
                status = org.basaltmc.luak.LuaThread.Companion.STATUS_DEAD
            }
        }
        ...
        fun lua_resume(new_thread: LuaThread, args: Varargs?): Varargs {
            ...
            try {
                ...
                previous_thread.state.status = org.basaltmc.luak.LuaThread.Companion.STATUS_NORMAL
                status = org.basaltmc.luak.LuaThread.Companion.STATUS_RUNNING
                finished = false
                ...
                return if (finished) {
                    ...
                    if (err != null) {
                        ...
                        if (err is ClosedCoroutine) LuaValue.TRUE!!
                            ...
                            new_thread.callstack?.let { stack ->
                                (stack as org.basaltmc.luak.lib.DebugLib.CallStack).frozen =
                    ...
                    status = org.basaltmc.luak.LuaThread.Companion.STATUS_SUSPENDED
                ...
                globals.running.state.status = org.basaltmc.luak.LuaThread.Companion.STATUS_RUNNING
