...
abstract class Varargs {
    ...
    internal class SubVarargs(private val v: Varargs, private val start: Int, private val end: Int) : Varargs() {
        ...
        override fun subargs(start: Int): Varargs? {
            ...
            if (start > 0) {
                ...
                if (newstart == this.end - 1) return org.basaltmc.luak.Varargs.PairVarargs(
                    v.arg(this.end - 1),
                    v.arg(this.end)
                )
                return org.basaltmc.luak.Varargs.SubVarargs(v, newstart, this.end)
            }
            return org.basaltmc.luak.Varargs.SubVarargs(v, newstart, this.end)
        }
    }
    ...
    internal class ArrayVarargs(v: Array<out LuaValue?>, r: Varargs) : Varargs() {
        ...
        override fun subargs(start: Int): Varargs? {
            ...
            if (start == 1) return this
            if (start > v.size) return r.subargs(start - v.size)
            return LuaValue.varargsOf(v, start - 1, v.size - (start - 1), r)
        }
    ...
    fun dealias(): Varargs {
        ...
        when (n) {
            ...
            1 -> return arg1()
            2 -> return org.basaltmc.luak.Varargs.PairVarargs(arg1(), (arg(2))!!)
            else -> {
                val v: Array<LuaValue?> = arrayOfNulls<LuaValue>(n)
                copyto(v, 0, n)
                return org.basaltmc.luak.Varargs.ArrayVarargs(v, (LuaValue.NONE)!!)
            }
        }
    }
}
