...
import org.basaltmc.luak.lib.DebugLib.CallFrame
 ...
 * [org.basaltmc.luak.compiler.LuaC]:
...
class LuaClosure(p: Prototype, env: LuaValue?) : LuaFunction() {
    ...
    override fun initupvalue1(env: LuaValue?) {
        ...
        if (descs == null || descs.isEmpty()) {
            this.upValues = org.basaltmc.luak.LuaClosure.Companion.NOUPVALUES
            return
        }
    ...
    fun errorHook(le: LuaError) {
        ...
        val debuglib: org.basaltmc.luak.lib.DebugLib? =
    ...
    private fun enrichIndexError(le: LuaError, p: Prototype, pc: Int) {
        ...
        when (Lua.GET_OPCODE(instr)) {
            Lua.OP_GETTABLE, Lua.OP_SELF -> {
                val found = org.basaltmc.luak.lib.DebugLib.getobjname(p, pc, Lua.GETARG_B(instr)) ?: return
                kind = found.namewhat
                name = found.name
            }

            Lua.OP_SETTABLE -> {
                val found = org.basaltmc.luak.lib.DebugLib.getobjname(p, pc, Lua.GETARG_A(instr)) ?: return
                kind = found.namewhat
                name = found.name
            }
    ...
    private fun enrichCallError(le: LuaError, p: Prototype, pc: Int) {
        ...
        val found = when (opcode) {
            Lua.OP_CALL, Lua.OP_TAILCALL ->
                org.basaltmc.luak.lib.DebugLib.getobjname(p, pc, Lua.GETARG_A(instr))
            // The one other instruction that calls: a generic `for` steps its
            // iterator, and Lua names that as what it is.
            Lua.OP_TFORCALL -> org.basaltmc.luak.lib.DebugLib.NameWhat("for iterator", "for iterator")
            else -> return
        } ?: return
    ...
    private fun enrichOperandError(le: LuaError, p: Prototype, pc: Int, stack: Array<LuaValue>) {
        ...
        for (rk in operands) {
            ...
            if (insertAt != m.length && org.basaltmc.luak.luaHasIntegerRepresentation(value)) continue
            val kind: String
            val name: String
            if (Lua.ISK(rk)) {
                kind = "constant"
                name = org.basaltmc.luak.lib.DebugLib.kname(p, pc, rk)
            } else {
                val found = org.basaltmc.luak.lib.DebugLib.getobjname(p, pc, rk) ?: return
                kind = found.namewhat
                name = found.name
            }
    ...
    private fun enrichArgError(le: LuaError, p: Prototype, pc: Int, stack: Array<LuaValue>) {
        ...
        if (m.startsWith("bad argument: ") && a < stack.size &&
            stack[a] is org.basaltmc.luak.lib.OneArgFunction
        ) {
        ...
        val nw = org.basaltmc.luak.lib.DebugLib.getobjname(p, pc, a)
    ...
    private fun processErrorHooks(le: LuaError, p: Prototype, pc: Int) {
        ...
        run {
            ...
            val debuglib: org.basaltmc.luak.lib.DebugLib? = globals?.debuglib
            if (debuglib != null) {
    ...
    private fun makeclosure(stack: Array<LuaValue>, i: Int, openups: Array<UpValue?>?): LuaClosure {
        ...
        val ncl = org.basaltmc.luak.LuaClosure(newp, globals)
