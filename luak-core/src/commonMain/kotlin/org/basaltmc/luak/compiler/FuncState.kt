...
import org.basaltmc.luak.Upvaldesc
import org.basaltmc.luak.compiler.LexState.ConsControl
import org.basaltmc.luak.compiler.LexState.expdesc
...
internal class FuncState internal constructor() : Constants() {
	...
	*/
    ...
    fun indexed(t: expdesc, k: expdesc) {
        ...
        _assert(t.k === LexState.VUPVAL || org.basaltmc.luak.compiler.FuncState.Companion.vkisinreg(t.k))
    ...
    fun constfolding(op: Int, e1: expdesc, e2: expdesc): Boolean {
        ...
        when (op) {
            OP_BAND, OP_BOR, OP_BXOR, OP_SHL, OP_SHR ->
                if (!org.basaltmc.luak.luaHasIntegerRepresentation(v1) ||
                    !org.basaltmc.luak.luaHasIntegerRepresentation(v2)
                ) return false

            OP_BNOT -> if (!org.basaltmc.luak.luaHasIntegerRepresentation(v1)) return false
        }
        when (op) {
            OP_ADD -> r = v1.add(v2)
            OP_SUB -> r = v1.sub(v2)
