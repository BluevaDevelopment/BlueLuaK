...
import org.basaltmc.luak.io.OutputStream
...
class DumpState(w: OutputStream?, strip: Boolean) {
    ...
    private var NUMBER_FORMAT: Int = org.basaltmc.luak.compiler.DumpState.Companion.NUMBER_FORMAT_DEFAULT
        ...
        while (i < n) {
            ...
            when (o.type()) {
                ...
                LuaValue.TNUMBER -> when (NUMBER_FORMAT) {
                    org.basaltmc.luak.compiler.DumpState.Companion.NUMBER_FORMAT_FLOATS_OR_DOUBLES ->
                        if (o.isinttype()) {
                            // Tagged apart from a float, or the subtype would
                            // not survive the round trip.
                            writer!!.write(org.basaltmc.luak.LoadState.LUA_TNUMINT)
                            dumpLong(o.tolong())
                        } else {
                    ...
                    org.basaltmc.luak.compiler.DumpState.Companion.NUMBER_FORMAT_INTS_ONLY -> {
                        kotlin.require(!(!org.basaltmc.luak.compiler.DumpState.Companion.ALLOW_INTEGER_CASTING && !o.isint())) { "not an integer: " + o }
                        writer!!.write(LuaValue.TNUMBER)
                        dumpInt(o.toint())
                    }

                    org.basaltmc.luak.compiler.DumpState.Companion.NUMBER_FORMAT_NUM_PATCH_INT32 -> if (o.isint()) {
    ...
    fun dumpHeader() {
        ...
        writer!!.write(org.basaltmc.luak.compiler.DumpState.Companion.SIZEOF_INT)
        dumpInt(LoadState.LUAC_INT)
        writer!!.write(org.basaltmc.luak.compiler.DumpState.Companion.SIZEOF_INSTRUCTION)
        dumpInt(LoadState.LUAC_INST)
        writer!!.write(org.basaltmc.luak.compiler.DumpState.Companion.SIZEOF_LUA_INTEGER)
        ...
        if (NUMBER_FORMAT == org.basaltmc.luak.compiler.DumpState.Companion.NUMBER_FORMAT_INTS_ONLY) {
            dumpInt(LoadState.LUAC_INT)
        } else {
    ...
    companion object {
        ...
        val NUMBER_FORMAT_DEFAULT: Int = org.basaltmc.luak.compiler.DumpState.Companion.NUMBER_FORMAT_FLOATS_OR_DOUBLES
	...
	*/
        ...
        fun dump(f: Prototype, w: OutputStream?, strip: Boolean): Int {
            val D: DumpState = org.basaltmc.luak.compiler.DumpState(w, strip)
            D.dumpHeader()
            D.dumpFunction(f)
            return D.status
        }
        ...
        fun dump(f: Prototype, w: OutputStream?, stripDebug: Boolean, numberFormat: Int, littleendian: Boolean): Int {
            when (numberFormat) {
                org.basaltmc.luak.compiler.DumpState.Companion.NUMBER_FORMAT_FLOATS_OR_DOUBLES, org.basaltmc.luak.compiler.DumpState.Companion.NUMBER_FORMAT_INTS_ONLY, org.basaltmc.luak.compiler.DumpState.Companion.NUMBER_FORMAT_NUM_PATCH_INT32 -> {}
                else -> throw IllegalArgumentException("number format not supported: " + numberFormat)
            }
            val D: DumpState = org.basaltmc.luak.compiler.DumpState(w, stripDebug)
            D.IS_LITTLE_ENDIAN = littleendian
            D.NUMBER_FORMAT = numberFormat
            D.SIZEOF_LUA_NUMBER =
                (if (numberFormat == org.basaltmc.luak.compiler.DumpState.Companion.NUMBER_FORMAT_INTS_ONLY) 4 else 8)
            D.dumpHeader()
            D.dumpFunction(f)
            return D.status
        }
