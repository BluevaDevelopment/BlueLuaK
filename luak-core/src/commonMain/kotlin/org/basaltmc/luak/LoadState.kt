...
import org.basaltmc.luak.io.DataInputStream
 ...
 * [org.basaltmc.luak.compiler.LuaC] compiler.
 * 
 * 
 ...
 * 
 * 
 * By default, when using [org.basaltmc.luak.lib.LuaPlatform] or
 * [org.basaltmc.luak.lib.jvm.JvmPlatform]
 * to construct globals, the [LoadState] default undumper is installed
 * as the default [Globals.Undumper].
 * 
 * 
 * 
 * A lua binary file is created via the [org.basaltmc.luak.compiler.DumpState] class
 * :
 * <pre> `Globals globals = JvmPlatform.standardGlobals(); Prototype p = globals.compilePrototype(new StringReader("print('hello, world')"), "main.lua"); ByteArrayOutputStream o = new ByteArrayOutputStream(); org.basaltmc.luak.compiler.DumpState.dump(p, o, false); byte[] lua_binary_file_bytes = o.toByteArray(); ` </pre>
 * 
 ...
 * @see LuaFunction
 * 
 * @see org.basaltmc.luak.compiler.LuaC
 * 
 * @see org.basaltmc.luak.luajc.LuaJC
 * 
 * @see Globals.compiler
 * 
 * @see Globals.load
...
class LoadState private constructor(
    ...
    fun loadIntArray(): IntArray {
        val n = loadInt()
        if (n == 0) return org.basaltmc.luak.LoadState.Companion.NOINTS
    ...
    fun loadNumber(): LuaValue {
        if (luacNumberFormat == org.basaltmc.luak.LoadState.Companion.NUMBER_FORMAT_INTS_ONLY) {
            return LuaInteger.valueOf(loadInt())!!
        } else {
            return org.basaltmc.luak.LoadState.Companion.longBitsToLuaNumber(loadInt64())
        }
    }
    ...
    fun loadConstants(f: Prototype) {
        ...
        val values: Array<LuaValue?> =
            if (n > 0) arrayOfNulls<LuaValue>(n) else org.basaltmc.luak.LoadState.Companion.NOVALUES
        for (i in 0..<n) {
            when (`is`.readByte().toInt()) {
                org.basaltmc.luak.LoadState.Companion.LUA_TNIL -> values[i] = LuaValue.NIL
                org.basaltmc.luak.LoadState.Companion.LUA_TBOOLEAN -> values[i] =
                    (if (0 != `is`.readUnsignedByte()) LuaValue.TRUE else LuaValue.FALSE)

                org.basaltmc.luak.LoadState.Companion.LUA_TINT -> values[i] = LuaInteger.valueOf(loadInt())!!
                org.basaltmc.luak.LoadState.Companion.LUA_TNUMINT -> values[i] = LuaInteger.valueOf(loadInt64())!!
                org.basaltmc.luak.LoadState.Companion.LUA_TNUMBER -> values[i] = loadNumber()
                org.basaltmc.luak.LoadState.Companion.LUA_TSTRING -> values[i] = loadString()
        ...
        val protos: Array<Prototype?>? =
            if (n > 0) arrayOfNulls<Prototype>(n) else org.basaltmc.luak.LoadState.Companion.NOPROTOS
    ...
    fun loadUpvalues(f: Prototype) {
        val n = loadInt()
        f.upvalues = if (n > 0) arrayOfNulls<Upvaldesc>(n) else org.basaltmc.luak.LoadState.Companion.NOUPVALDESCS
        for (i in 0..<n) {
    ...
    fun loadDebug(f: Prototype) {
        ...
        var n = loadInt()
        f.locvars = if (n > 0) arrayOfNulls<LocVars>(n) else org.basaltmc.luak.LoadState.Companion.NOLOCVARS
        for (i in 0..<n) {
...
//		 this.L.pop();
     ...
     */
    @kotlin.Throws(IOException::class)
    /** Reads back what [org.basaltmc.luak.compiler.DumpState.dumpHeader] wrote, refusing anything else. */
    fun loadHeader() {
        ...
        if (luacVersion != org.basaltmc.luak.LoadState.Companion.LUAC_VERSION) {
            badformat("version mismatch")
        }
        luacFormat = `is`.readByte().toInt() and 0xFF
        if (luacFormat != org.basaltmc.luak.LoadState.Companion.LUAC_FORMAT) {
            badformat("format mismatch")
        }
        for (i in org.basaltmc.luak.LoadState.Companion.LUAC_TAIL.indices) {
            if (`is`.readByte() != org.basaltmc.luak.LoadState.Companion.LUAC_TAIL[i]) {
        ...
        luacLittleEndian = when (org.basaltmc.luak.LoadState.Companion.LUAC_INT) {
            little -> true
            big -> false
            else -> badformat("corrupted chunk")
        }
        checksize(4, "instruction")
        checkvalue(loadInt().toLong(), org.basaltmc.luak.LoadState.Companion.LUAC_INST.toLong())
        checksize(8, "integer")
        checkvalue(loadInt64(), org.basaltmc.luak.LoadState.Companion.LUAC_INT.toLong())
        ...
        when (luacSizeofLuaNumber) {
            4 -> {
                luacNumberFormat = org.basaltmc.luak.LoadState.Companion.NUMBER_FORMAT_INTS_ONLY
                checkvalue(loadInt().toLong(), org.basaltmc.luak.LoadState.Companion.LUAC_INT.toLong())
            }

            8 -> {
                luacNumberFormat = org.basaltmc.luak.LoadState.Companion.NUMBER_FORMAT_FLOATS_OR_DOUBLES
                if (loadInt64() != org.basaltmc.luak.LoadState.Companion.LUAC_NUM.toBits()) {
    ...
    private class GlobalsUndumper : Globals.Undumper {
        ...
        override fun undump(stream: InputStream?, chunkname: String?): Prototype? {
            return org.basaltmc.luak.LoadState.Companion.undump(stream!!, chunkname!!)
        }
    }

    companion object {
        /** Shared instance of Globals.Undumper to use loading prototypes from binary lua files  */
        val instance: Globals.Undumper = org.basaltmc.luak.LoadState.GlobalsUndumper()
        ...
        fun install(globals: Globals) {
            globals.undumper = org.basaltmc.luak.LoadState.Companion.instance
        }
        ...
        fun undump(stream: InputStream, chunkname: String): Prototype? {
            val sname: String? = org.basaltmc.luak.LoadState.Companion.getSourceName(chunkname)
            ...
            val s: LoadState = org.basaltmc.luak.LoadState(stream, sname)
            try {
                s.loadHeader()
        ...
        fun getSourceName(name: String): String? {
            ...
            else if (name.startsWith("\u001b")) sname = org.basaltmc.luak.LoadState.Companion.SOURCE_BINARY_STRING
            return sname
        }
    }
}
