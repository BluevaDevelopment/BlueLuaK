...
import org.basaltmc.luak.lib.BaseLib
import org.basaltmc.luak.lib.DebugLib
import org.basaltmc.luak.lib.MathLib
import org.basaltmc.luak.lib.PackageLib
import org.basaltmc.luak.lib.ResourceFinder
import org.basaltmc.luak.io.IOException
import org.basaltmc.luak.io.InputStream
import org.basaltmc.luak.io.PrintStream
import org.basaltmc.luak.io.Reader
import org.basaltmc.luak.io.standardError
 ...
 * 
 * 
 * There are alternate flows when the direct lua-to-Java bytecode compiling [org.basaltmc.luak.luajc.LuaJC] is used.
 * 
 *  * compile lua to lua bytecode using [Compiler] or load precompiled code using [Undumper]
 *  * convert lua bytecode to equivalent Java bytecode using [org.basaltmc.luak.luajc.LuaJC] that implements [Loader] directly
 * 
 * 
 * <h3>Java Field</h3>
 ...
 * When using [org.basaltmc.luak.lib.LuaPlatform] or [org.basaltmc.luak.lib.jvm.JvmPlatform],
...
class Globals : LuaTable() {
    ...
    fun compile(script: String, chunkname: String?): Prototype =
        try {
            compilePrototype(org.basaltmc.luak.Globals.StrReader(script), chunkname)!!
        } catch (l: LuaError) {
            throw l
        } catch (e: Exception) {
    ...
    fun load(script: String, chunkname: String?): LuaValue? {
        return load(org.basaltmc.luak.Globals.StrReader(script), chunkname)
    }
    ...
    fun load(script: String): LuaValue? {
        return load(org.basaltmc.luak.Globals.StrReader(script), script)
    }
    ...
    fun load(script: String, chunkname: String?, environment: LuaTable?): LuaValue? {
        return load(org.basaltmc.luak.Globals.StrReader(script), chunkname, environment)
    }
    ...
    fun load(reader: Reader, chunkname: String?): LuaValue? {
        return load(org.basaltmc.luak.Globals.UTF8Stream(reader), chunkname, "t", this)
    }
    ...
    fun load(reader: Reader, chunkname: String?, environment: LuaTable?): LuaValue? {
        return load(org.basaltmc.luak.Globals.UTF8Stream(reader), chunkname, "t", environment)
    }
    ...
    fun loadPrototype(`is`: InputStream, chunkname: String?, mode: String): Prototype? {
        ...
        if (!`is`.markSupported()) `is` = org.basaltmc.luak.Globals.BufferedStream(`is`)
        `is`.mark(4)
    ...
    fun compilePrototype(reader: Reader, chunkname: String?): Prototype? {
        return compilePrototype(org.basaltmc.luak.Globals.UTF8Stream(reader), chunkname)
    }
