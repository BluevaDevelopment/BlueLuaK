...
import org.basaltmc.luak.LuaTable.StrongSlot
...
class WeakTable(private val weakkeys: Boolean, private val weakvalues: Boolean, backing: LuaValue?) : Metatable {
    ...
    override fun entry(key: LuaValue?, value: LuaValue?): Slot? {
        ...
        if (weakkeys && !(key!!.isnumber() || key.isstring() || key.isboolean())) {
            if (weakvalues && !(value.isnumber() || value.isstring() || value.isboolean())) {
                return org.basaltmc.luak.WeakTable.WeakKeyAndValueSlot(key!!, value, null)
            } else {
                return org.basaltmc.luak.WeakTable.WeakKeySlot(key!!, value, null)
            }
        }
        if (weakvalues && !(value.isnumber() || value.isstring() || value.isboolean())) {
            return org.basaltmc.luak.WeakTable.WeakValueSlot(key, value, null)
        }
    ...
    internal class WeakKeySlot : WeakSlot {
        ...
        constructor(
            ...
            org.basaltmc.luak.WeakTable.Companion.weaken(key),
            org.basaltmc.luak.WeakTable.Companion.weaken(value!!),
            next,
        ) {
            keyhash = key.hashCode()
            org.basaltmc.luak.WeakTable.Companion.pin(key, value)
        }
        ...
        override fun set(value: LuaValue?): Slot? {
            val key: LuaValue? = strongkey()
            if (key != null) {
                org.basaltmc.luak.WeakTable.Companion.unpin(
                    key,
                    org.basaltmc.luak.WeakTable.Companion.strengthen(this.value),
                )
                if (value != null) org.basaltmc.luak.WeakTable.Companion.pin(key, value)
            }
            this.value = if (value == null) null else org.basaltmc.luak.WeakTable.Companion.weaken(value)
            return this
        }

        override fun strongkey(): LuaValue? {
            return org.basaltmc.luak.WeakTable.Companion.strengthen(key)
        }

        override fun strongvalue(): LuaValue? {
            return org.basaltmc.luak.WeakTable.Companion.strengthen(value)
        }

        override fun copy(rest: Slot?): WeakSlot {
            return org.basaltmc.luak.WeakTable.WeakKeySlot(this, rest)
        }
    }
    ...
    internal class WeakValueSlot : WeakSlot {
        ...
        override fun set(value: LuaValue?): Slot? {
            this.value = org.basaltmc.luak.WeakTable.Companion.weaken((value)!!)
            return this
        }
        ...
        override fun copy(next: Slot?): WeakSlot {
            return org.basaltmc.luak.WeakTable.WeakValueSlot(this, next)
        }
    }
    ...
    internal class WeakKeyAndValueSlot : WeakSlot {
        ...
        override fun copy(next: Slot?): WeakSlot {
            return org.basaltmc.luak.WeakTable.WeakKeyAndValueSlot(this, next)
        }
    }
    ...
    override fun wrap(value: LuaValue?): LuaValue? {
        return if (weakvalues) org.basaltmc.luak.WeakTable.Companion.weaken(value!!) else value
    }

    override fun arrayget(array: Array<LuaValue?>?, index: Int): LuaValue? {
        var value: LuaValue? = array!![index]
        if (value != null) {
            value = org.basaltmc.luak.WeakTable.Companion.strengthen(value)
            if (value == null) {
                array[index] = null
            }
        }
    ...
    companion object {
        ...
        protected fun weaken(value: LuaValue): LuaValue {
            when (value.type()) {
                LuaValue.TFUNCTION, LuaValue.TTHREAD, LuaValue.TTABLE -> return org.basaltmc.luak.WeakTable.WeakValue(
                    value
                )

                LuaValue.TUSERDATA -> return org.basaltmc.luak.WeakTable.WeakUserdata(value)
                else -> return value
            }
        }
