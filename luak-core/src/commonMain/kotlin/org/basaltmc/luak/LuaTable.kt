...
import org.basaltmc.luak.WeakReference
...
open class LuaTable : LuaValue, Metatable {
    ...
    constructor() {
        array = NOVALS
        hash = org.basaltmc.luak.LuaTable.Companion.NOBUCKETS
    ...
    constructor(varargs: Varargs, firstarg: Int) {
        ...
        presize(n, 1)
        set(org.basaltmc.luak.LuaTable.Companion.N, valueOf(n))
    ...
    override fun presize(narray: Int) {
        ...
        if (narray > array.size) {
            ...
            array =
                org.basaltmc.luak.LuaTable.Companion.resize(array, 1 shl org.basaltmc.luak.LuaTable.Companion.log2(narray))
    ...
    fun presize(narray: Int, nhash: Int) {
        ...
        if (nhash > 0 && nhash < org.basaltmc.luak.LuaTable.Companion.MIN_HASH_CAPACITY) nhash =
            org.basaltmc.luak.LuaTable.Companion.MIN_HASH_CAPACITY
        // Size of both parts must be a power of two.
        array =
            (if (narray > 0) arrayOfNulls<LuaValue>(1 shl org.basaltmc.luak.LuaTable.Companion.log2(narray)) else NOVALS)
        hash =
            (if (nhash > 0) arrayOfNulls<Slot>(1 shl org.basaltmc.luak.LuaTable.Companion.log2(nhash)) else org.basaltmc.luak.LuaTable.Companion.NOBUCKETS)
        hashEntries = 0
    ...
    fun hashset(key: LuaValue, value: LuaValue) {
        if (value.isnil()) hashRemove(key)
            ...
            val entry: Slot? = if (m_metatable != null)
                ...
                org.basaltmc.luak.LuaTable.Companion.defaultEntry(key, value)
    ...
    private fun hashSlot(key: LuaValue): Int {
        return org.basaltmc.luak.LuaTable.Companion.hashSlot(key, hash.size - 1)
    }
        ...
        while (i < hash.size) {
            ...
            while (s != null) {
                ...
                if ((s.arraykey(Int.MAX_VALUE).also { k = it }) > 0) {
                    nums[org.basaltmc.luak.LuaTable.Companion.log2(k)]++
                    total++
                }
                s = s.rest()
            }
    ...
    private fun rehash(newKey: Int) {
        ...
        if (newKey > 0) {
            total++
            nums[org.basaltmc.luak.LuaTable.Companion.log2(newKey)]++
        }
        ...
        if (newArraySize != oldArray.size) {
            ...
            if (newArraySize > oldArray.size) {
                var i: Int = org.basaltmc.luak.LuaTable.Companion.log2(oldArray.size + 1)
                val j: Int = org.basaltmc.luak.LuaTable.Companion.log2(newArraySize) + 1
                while (i < j) {
                    movingToArray += nums[i]
                    ++i
                }
            } else if (oldArray.size > newArraySize) {
                var i: Int = org.basaltmc.luak.LuaTable.Companion.log2(newArraySize + 1)
                val j: Int = org.basaltmc.luak.LuaTable.Companion.log2(oldArray.size) + 1
                while (i < j) {
                    movingToArray -= nums[i]
                    ++i
                }
        ...
        if (newHashSize > 0) {
            ...
            newCapacity = if (newHashSize < org.basaltmc.luak.LuaTable.Companion.MIN_HASH_CAPACITY)
                org.basaltmc.luak.LuaTable.Companion.MIN_HASH_CAPACITY
            else
                1 shl org.basaltmc.luak.LuaTable.Companion.log2(newHashSize)
            newHashMask = newCapacity - 1
            newHash = arrayOfNulls<Slot>(newCapacity)
        } else {
            newCapacity = 0
            newHashMask = 0
            newHash = org.basaltmc.luak.LuaTable.Companion.NOBUCKETS
        }
        ...
        while (i < oldArray.size) {
            ...
            if ((oldArray[i++].also { v = it }) != null) {
                val slot: Int = org.basaltmc.luak.LuaTable.Companion.hashmod(LuaInteger.hashCode(i), newHashMask)
                val newEntry: Slot?
                if (m_metatable != null) {
                    ...
                    newEntry = org.basaltmc.luak.LuaTable.Companion.defaultEntry(valueOf(i), (v)!!)
                }
    ...
    override fun entry(key: LuaValue?, value: LuaValue?): Slot? {
        return org.basaltmc.luak.LuaTable.Companion.defaultEntry(key!!, value!!)
    }
    ...
    fun sort(comparator: LuaValue, debuglib: org.basaltmc.luak.lib.DebugLib? = null) {
     ...
     */
    private fun auxsort(from: Int, to: Int, cmpfunc: LuaValue?, debuglib: org.basaltmc.luak.lib.DebugLib?) {
        var lo = from
        var up = to
        while (lo < up) {
    ...
    private fun partition(
        ...
        up: Int,
        cmpfunc: LuaValue?,
        debuglib: org.basaltmc.luak.lib.DebugLib?,
    ): Int {
        val pivot: Int = up - 1
        var i: Int = lo
        var j: Int = up - 1
    ...
    private fun compare(
        ...
        j: Int,
        cmpfunc: LuaValue?,
        debuglib: org.basaltmc.luak.lib.DebugLib? = null,
    ): Boolean {
        ...
        if (cmpfunc != null) {
            ...
            return org.basaltmc.luak.lib.callback(debuglib, cmpfunc, varargsOf(a, b)!!).arg1()!!.toboolean()
        } else {
            return a.lt_b(b)
        }
    }
    ...
    private class LinkSlot(private var entry: Entry, next: Slot) : StrongSlot {
        ...
        override fun remove(target: StrongSlot?): Slot {
            if (this === target) {
                return org.basaltmc.luak.LuaTable.DeadSlot((key())!!, next)
            } else {
        ...
        override fun relink(rest: Slot?): Slot? {
            ...
            return if (rest != null) org.basaltmc.luak.LuaTable.LinkSlot(entry, rest) else entry as Slot?
        }
    ...
    abstract class Entry : Varargs(), StrongSlot {
        ...
        override fun add(entry: Slot?): Slot? {
            return org.basaltmc.luak.LuaTable.LinkSlot(this, (entry)!!)
        }

        override fun remove(target: StrongSlot?): Slot {
            return org.basaltmc.luak.LuaTable.DeadSlot((key())!!, null)
        }

        override fun relink(rest: Slot?): Slot {
            return if (rest != null) org.basaltmc.luak.LuaTable.LinkSlot(this, rest) else this as Slot
        }
    }
    ...
    internal class NormalEntry(key: LuaValue, value: LuaValue?) : Entry() {
        ...
        override fun keyindex(hashMask: Int): Int {
            return org.basaltmc.luak.LuaTable.Companion.hashSlot(key, hashMask)
        }
    ...
    private class IntKeyEntry(private val key: Long, value: LuaValue?) : Entry() {
        ...
        override fun keyindex(mask: Int): Int {
            return org.basaltmc.luak.LuaTable.Companion.hashmod(LuaInteger.hashCode(key), mask)
        }
    ...
    private class NumberValueEntry(key: LuaValue, value: Double) : Entry() {
        ...
        public override fun set(value: LuaValue?): Entry {
            ...
            return org.basaltmc.luak.LuaTable.NormalEntry(this.key, value)
        }

        override fun keyindex(mask: Int): Int {
            return org.basaltmc.luak.LuaTable.Companion.hashSlot(key, mask)
        }
    ...
    private class DeadSlot(key: LuaValue, private var next: Slot?) : Slot {
        ...
        init {
            this.key = if (org.basaltmc.luak.LuaTable.Companion.isLargeKey(key)) WeakReference(key) else key as Any
        }
    ...
    companion object {
        ...
        fun hashSlot(key: LuaValue, hashMask: Int): Int {
            when (key.type()) {
                TNUMBER, TTABLE, TTHREAD, TLIGHTUSERDATA, TUSERDATA -> return org.basaltmc.luak.LuaTable.Companion.hashmod(
                    key.hashCode(),
                    hashMask
                )

                else -> return org.basaltmc.luak.LuaTable.Companion.hashpow2(key.hashCode(), hashMask)
            }
        }
        ...
        internal fun defaultEntry(key: LuaValue, value: LuaValue): Entry {
            if (key.isinttype()) {
                return org.basaltmc.luak.LuaTable.IntKeyEntry(key.tolong(), value)
            } else if (value.type() === TNUMBER && !value.isinttype()) {
                return org.basaltmc.luak.LuaTable.NumberValueEntry(key, value.todouble())
            } else {
                return org.basaltmc.luak.LuaTable.NormalEntry(key, value)
            }
        }
