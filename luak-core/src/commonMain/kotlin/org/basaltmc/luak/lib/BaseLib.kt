...
import org.basaltmc.luak.platformUsedMemory
import org.basaltmc.luak.platformCollectGarbage
...
import org.basaltmc.luak.io.ByteArrayInputStream
...
import org.basaltmc.luak.io.platformFilesSupported
import org.basaltmc.luak.io.platformOpenFile
import org.basaltmc.luak.io.platformResource
...
open class BaseLib : TwoArgFunction(), ResourceFinder {
    ...
    override fun call(modname: LuaValue?, env: LuaValue?): LuaValue? {
        ...
        env!!.set("assert", org.basaltmc.luak.lib.BaseLib._assert())
        env!!.set("collectgarbage", org.basaltmc.luak.lib.BaseLib.collectgarbage(this))
        env!!.set("warn", warn(this))
        env!!.set("dofile", dofile())
        env!!.set("error", org.basaltmc.luak.lib.BaseLib.error())
        env!!.set("getmetatable", org.basaltmc.luak.lib.BaseLib.getmetatable())
        env!!.set("load", load())
        env!!.set("loadfile", loadfile())
        env!!.set("pcall", pcall())
        env!!.set("print", print(this))
        env!!.set("rawequal", org.basaltmc.luak.lib.BaseLib.rawequal())
        env!!.set("rawget", org.basaltmc.luak.lib.BaseLib.rawget())
        env!!.set("rawlen", org.basaltmc.luak.lib.BaseLib.rawlen())
        env!!.set("rawset", org.basaltmc.luak.lib.BaseLib.rawset())
        env!!.set("select", org.basaltmc.luak.lib.BaseLib.select())
        env!!.set("setmetatable", org.basaltmc.luak.lib.BaseLib.setmetatable(this))
        env!!.set("tonumber", org.basaltmc.luak.lib.BaseLib.tonumber())
        env!!.set("tostring", org.basaltmc.luak.lib.BaseLib.tostring())
        env!!.set("type", org.basaltmc.luak.lib.BaseLib.type())
        env!!.set("xpcall", xpcall())

        val next: next?
        env!!.set("next", org.basaltmc.luak.lib.BaseLib.next().also { next = it })
        env!!.set("pairs", org.basaltmc.luak.lib.BaseLib.pairs((next)!!))
        env!!.set("ipairs", org.basaltmc.luak.lib.BaseLib.ipairs())
    ...
    internal class collectgarbage(private val baselib: BaseLib) : VarArgFunction() {
        ...
        override fun invoke(args: Varargs): Varargs {
            ...
            if ("collect".equals(s)) {
                ...
                org.basaltmc.luak.lib.BaseLib.collectgarbage.stepped += if (size > 0) size else 1
                if (org.basaltmc.luak.lib.BaseLib.collectgarbage.stepped <
                    org.basaltmc.luak.lib.BaseLib.collectgarbage.CYCLE
                ) {
                    return (LuaValue.FALSE)!!
                }
                org.basaltmc.luak.lib.BaseLib.collectgarbage.stepped = 0
                baselib.globals!!.memory.collected()
                ...
                val previous: Long = org.basaltmc.luak.lib.BaseLib.collectgarbage.parameters[name]
                    ?: return (argerror(2, "invalid option '" + name + "'"))!!
                if (!args.isnoneornil(3)) {
                    org.basaltmc.luak.lib.BaseLib.collectgarbage.parameters[name] = args.checklong(3)
                }
                ...
                val previous: String = org.basaltmc.luak.lib.BaseLib.collectgarbage.mode
                org.basaltmc.luak.lib.BaseLib.collectgarbage.mode = s!!
                return valueOf(previous)!!
            } else {
    ...
    companion object {
        ...
        internal fun tolstring(value: LuaValue): LuaValue {
            ...
            val name: LuaValue = value.metatag(org.basaltmc.luak.LuaValue.NAME)
            if (name.isstring()) {
        ...
        override fun invoke(args: Varargs): Varargs {
            ...
            return loadStream(
                if (ld.isstring()) ld.strvalue()
                    !!.toInputStream() else org.basaltmc.luak.lib.BaseLib.StringInputStream((ld.checkfunction())!!),
                source,
                mode,
                env
            )
    ...
    internal inner class pcall : VarArgFunction() {
        override fun invoke(args: Varargs): Varargs {
            ...
            try {
                ...
                // some stack back.
                if (!org.basaltmc.luak.platformIsStackOverflow(deep)) throw deep
    ...
    internal inner class print(val baselib: BaseLib) : VarArgFunction() {
        override fun invoke(args: Varargs): Varargs {
            ...
            while (i <= n) {
                ...
                val s: LuaString = org.basaltmc.luak.lib.BaseLib.tolstring(args.arg(i)!!).strvalue()!!
    ...
    internal class tostring : LibFunction() {
        override fun call(arg: LuaValue?): LuaValue? {
            return org.basaltmc.luak.lib.BaseLib.tolstring(arg!!)
        }
    }
    ...
    internal inner class xpcall : VarArgFunction() {
        override fun invoke(args: Varargs): Varargs {
            ...
            try {
                try {
                    ...
                    if (!org.basaltmc.luak.platformIsStackOverflow(overflow)) throw overflow
        ...
        private fun runMessageHandler(t: LuaThread, errval: LuaValue): LuaValue {
            ...
            try {
                if (handler !is org.basaltmc.luak.LuaClosure) t.state.foreigncalls++
                t.state.inhandler++
    ...
    internal class ipairs : VarArgFunction() {
        var inext: inext = org.basaltmc.luak.lib.BaseLib.inext()
        override fun invoke(args: Varargs): Varargs {
...
private fun protectedCall(t: LuaThread?, f: LuaValue, args: Varargs): Varargs {
    ...
    if (debuglib != null) debuglib.onCall(f as org.basaltmc.luak.LuaFunction)
    try {
        enterForeign(state)
...
private fun frameFor(t: LuaThread?, f: LuaValue): DebugLib? {
    if (f !is org.basaltmc.luak.LuaFunction || f is org.basaltmc.luak.LuaClosure) return null
...
private fun enterForeign(state: LuaThread.State) {
    org.basaltmc.luak.enterForeignCall(state)
}
